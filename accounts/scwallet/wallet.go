...
// 如果打开智能卡需要使用配对密码进行配对，则返回 ErrPairingPasswordNeeded。在这种情况下，调用应用程序应请求用户输入配对密码并将其发回。
...
// 如果打开智能卡需要 PIN 码，则返回 ErrPINNeeded。在这种情况下，调用应用程序应请求用户输入 PIN 码并将其发回。
...
// 如果打开智能卡需要 PIN 码，但所有 PIN 尝试均已用尽，则返回 ErrPINUnblockNeeded。在这种情况下，调用应用程序应请求用户输入 PUK 和新的 PIN 码以设置卡。
...
// 如果尝试打开智能卡，但已经存在配对且解锁的会话，则返回 ErrAlreadyOpen。
...
// 如果从签名恢复的公钥与用户期望的公钥不匹配，则返回 ErrPubkeyMismatch。
...
var (
	...
	// DerivationSignatureHash 用于从此哈希的签名派生公钥
...
// Wallet 代表一个智能卡钱包实例。
type Wallet struct {
	Hub       *Hub   // A handle to the Hub that instantiated this wallet. // 实例化该钱包的 Hub 的句柄。
	PublicKey []byte // The wallet's public key (used for communication and identification, not signing!) // 钱包的公钥（用于通信和识别，而不是签名！）

	lock    sync.Mutex // Lock that gates access to struct fields and communication with the card // 锁定对结构字段的访问以及与卡的通信
	card    *pcsc.Card // A handle to the smartcard interface for the wallet. // 钱包智能卡接口的句柄。
	session *Session   // The secure communication session with the card // 与卡的安全通信会话
...
// transmit sends an APDU to the smartcard and receives and decodes the response. It automatically handles requests by the card to fetch the return data separately, and returns an error if the response status code is not success.
// 传输将 APDU 发送到智能卡并接收和解码响应。它自动处理卡的请求，单独获取返回数据，如果响应状态码不成功，则返回错误。
...
// applicationInfo encodes information about the smartcard application - its instance UID and public key.
// applicationInfo 对有关智能卡应用程序的信息进行编码 - 其实例 UID 和公钥。
type applicationInfo struct {
	InstanceUID []byte `asn1:"tag:15"`
	PublicKey   []byte `asn1:"tag:0"`
}

// connect connects to the wallet application and establishes a secure channel with it. must be called before any other interaction with the wallet.
// connect 连接到钱包应用程序并与其建立安全通道。必须在与钱包进行任何其他交互之前调用。
...
// doselect 是一个内部（未锁定）函数，用于将 SELECT APDU 发送到卡。
...
// pair is an internal (unlocked) function for establishing a new pairing with the wallet.
// pair 是一个内部（解锁）功能，用于与钱包建立新的配对。
...
// URL retrieves the canonical path under which this wallet is reachable. It is user by upper layers to define a sorting order over all wallets from multiple backends.
// URL 检索可访问此钱包的规范路径。由上层用户定义来自多个后端的所有钱包的排序顺序。
func (w *Wallet) URL() accounts.URL {
	return accounts.URL{
		Scheme: w.Hub.scheme,
		Path:   fmt.Sprintf("%x", w.PublicKey[1:5]), // Byte #0 isn't unique; 1:5 covers << 64K cards, bump to 1:9 for << 4M // 字节 #0 不是唯一的； 1:5 涵盖 `<``<` 64K 卡，对于 `<``<` 4M 卡则升至 1:9
	}
}
...
func (w *Wallet) Open(passphrase string) error {
	...
	// If the smart card is not yet paired, attempt to do so either from a previous pairing key or form the supplied PUK code.
	// 如果智能卡尚未配对，请尝试使用之前的配对密钥或形成提供的 PUK 码进行配对。
	if !w.session.paired() {
		...
		if pairing := w.Hub.pairing(w); pairing != nil {
			...
			// Pairing succeeded, fall through to PIN checks. This will of course fail, but we can't return ErrPINNeeded directly here because we don't know whether a PIN check or a PIN reset is needed.
			// 配对成功，进入 PIN 检查。这当然会失败，但我们不能在这里直接返回 ErrPINNeeded，因为我们不知道是否需要 PIN 检查或 PIN 重置。
			passphrase = ""
		}
	}
	// The smart card was successfully paired, retrieve its status to check whether PIN verification or unblocking is needed.
	// 智能卡已成功配对，检索其状态以检查是否需要 PIN 验证或解锁。
	status, err := w.session.walletStatus()
	if err != nil {
		return err
	}
	// Request the appropriate next authentication data, or use the one supplied
	// 请求适当的下一个身份验证数据，或使用提供的数据
	switch {
...
// 初始化将从提供的密钥生成的密钥对安装到钱包中。
func (w *Wallet) Initialize(seed []byte) error {
	go w.selfDerive()
	// DO NOT lock at this stage, as the initialize function relies on Status()
	// 在此阶段不要锁定，因为初始化函数依赖于 Status()
...
// SignDataWithPassphrase 请求钱包使用给定的密码作为额外的身份验证信息来签署给定的哈希值。
// It looks up the account specified either solely via its address contained within, or optionally with the aid of any location metadata from the embedded URL field.
// 它可以仅通过其中包含的地址查找指定的帐户，也可以选择借助嵌入的 URL 字段中的任何位置元数据来查找指定的帐户。
...
// 如果钱包需要额外的身份验证来签署请求（例如用于解密帐户的密码，或用于验证交易的 PIN 码），则将返回 AuthNeededError 实例，其中包含用户需要哪些字段或操作的信息。用户可以通过 SignHashWithPassphrase 或其他方式（例如在密钥库中解锁帐户）提供所需的详细信息来重试。
...
// SignTextWithPassphrase 实现accounts.Wallet，尝试使用密码短语作为额外身份验证使用给定帐户对给定哈希进行签名
...
// SignTxWithPassphrase 请求钱包签署给定的交易，并使用给定的密码作为额外的身份验证信息。
// It looks up the account specified either solely via its address contained within, or optionally with the aid of any location metadata from the embedded URL field.
// 它可以仅通过其中包含的地址查找指定的帐户，也可以选择借助嵌入的 URL 字段中的任何位置元数据来查找指定的帐户。
...
// findAccountPath returns the derivation path for the provided account. It first checks for the address in the list of pinned accounts, and if it is not found, attempts to parse the derivation path from the account's URL.
// findAccountPath 返回所提供帐户的派生路径。它首先检查固定帐户列表中的地址，如果未找到，则尝试从帐户的 URL 解析派生路径。
...
// 会话代表与钱包的安全通信会话。
type Session struct {
	Wallet   *Wallet               // A handle to the wallet that opened the session // 打开会话的钱包的句柄
	Channel  *SecureChannelSession // A secure channel for encrypted messages // 加密消息的安全通道
	verified bool                  // Whether the pin has been verified in this session. // PIN是否已在本次会话中得到验证。
}

// pair establishes a new pairing over this channel, using the provided secret.
// pair 使用提供的秘密通过该通道建立新的配对。
...
// unblockPin unblocks a wallet with the provided puk and resets the pin to the new one specified.
// unblockPin 使用提供的 puk 解锁钱包，并将 PIN 码重置为指定的新 PIN 码。
...
type walletStatus struct {
	PinRetryCount int  // Number of remaining PIN retries // 剩余 PIN 码重试次数
	PukRetryCount int  // Number of remaining PUK retries // 剩余 PUK 重试次数
	Initialized   bool // Whether the card has been initialized with a private key // 卡是否已用私钥初始化
}

// walletStatus fetches the wallet's status from the card.
// walletStatus 从卡中获取钱包的状态。
...
// derivationPath 从卡中获取钱包当前的派生路径。
//lint:ignore U1000 needs to be added to the console interface
//lint:ignore U1000需要添加到console界面
...
func (s *Session) initialize(seed []byte) error {
	// Check that the wallet isn't currently initialized, otherwise the key would be overwritten.
	// 检查钱包当前是否未初始化，否则密钥将被覆盖。
...
// signatureData contains information on a signature - the signature itself and the corresponding public key.
// SignatureData 包含有关签名的信息 - 签名本身和相应的公钥。
...
// sign asks the card to sign a message, and returns a valid signature after recovering the v value.
// sign要求卡对消息进行签名，恢复v值后返回有效签名。
...
// informPublicKey 确认给定的签名属于指定的密钥。
func confirmPublicKey(sig, pubkey []byte) error {
	_, err := makeRecoverableSignature(DerivationSignatureHash[:], sig, pubkey)
	return err
}

// makeRecoverableSignature uses a signature and an expected public key to recover the v value and produce a recoverable signature.
// makeRecoverableSignature 使用签名和预期的公钥来恢复 v 值并生成可恢复的签名。
