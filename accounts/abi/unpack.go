...
func ReadInteger(typ Type, b []byte) (interface{}, error) {
	...
	// big.SetBytes can't tell if a number is negative or positive in itself. On EVM, if the returned number > max int256, it is negative. A number is > max int256 if the bit at position 255 is set.
	// big.SetBytes 无法判断数字本身是负数还是正数。在 EVM 上，如果返回的数字 > max int256，则为负数。如果设置了位置 255 处的位，则数字 > max int256。
	if ret.Bit(255) == 1 {
...
// readFunctionType 通过始终将其呈现为 24 数组（地址 + sig = 24 字节）来强制执行该标准
...
func forEachUnpack(t Type, output []byte, start, size int) (interface{}, error) {
	...
	// Arrays have packed elements, resulting in longer unpack steps. Slices have just 32 bytes per element (pointing to the contents).
	// 数组具有打包元素，导致解包步骤更长。切片的每个元素只有 32 个字节（指向内容）。
...
// toGoType 解析输出字节，并根据 ABI 规范递归地将这些字节的值分配给 go 类型。
func toGoType(index int, t Type, output []byte) (interface{}, error) {
	...
	case StringTy: // variable arrays are written at the end of the return bytes // 变量数组写在返回字节的末尾
...
// lengthPrefixPointsTo 将 32 字节切片解释为偏移量，然后确定要查找哪些索引来解码类型。
