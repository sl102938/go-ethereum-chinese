...
// TestReplicate can be used to replicate crashers from the fuzzing tests. Just replace testString with the data in .quoted
// TestReplicate 可用于从模糊测试中复制崩溃程序。只需将 testString 替换为 .quoted 中的数据即可
...
func unpackPack(abi ABI, method string, input []byte) ([]interface{}, bool) {
	if out, err := abi.Unpack(method, input); err == nil {
		...
		if err != nil {
			...
			// 我们有一些误报，因为我们可以成功解压这些类型，但不能打包它们
...
func fuzzAbi(input []byte) {
	...
	for i := 0; i < upTo(fuzzer, 10); i++ {
		...
		case 0: // 10% chance to make it a slice // 10% 的机会将其切成薄片
			argTyp += "[]"
		case 1: // 10% chance to make it an array // 10% 的机会使其成为数组
