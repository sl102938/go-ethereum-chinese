...
// "0x00Ce0d46d924CC8437c806721496599FC3FFA268", 2218516807680, "usd"
// “0x00Ce0d46d924CC8437c806721496599FC3FFA268”，2218516807680，“美元”
...
// TestEventMultiValueWithArrayUnpack verifies that array fields will be counted after parsing array.
// TestEventMultiValueWithArrayUnpack 验证解析数组后是否会对数组字段进行计数。
...
func TestEventTupleUnpack(t *testing.T) {
	...
	type EventTransferWithTag struct {
		// this is valid because `value` is not exportable, so value is only unmarshalled into `Value1`.
		// 这是有效的，因为“value”不可导出，因此值仅被解组到“Value1”中。
...
// TestEventUnpackIndexed 验证事件解码器将跳过索引字段。
func TestEventUnpackIndexed(t *testing.T) {
	t.Parallel()
	definition := `[{"name": "test", "type": "event", "inputs": [{"indexed": true, "name":"value1", "type":"uint8"},{"indexed": false, "name":"value2", "type":"uint8"}]}]`
	type testStruct struct {
		Value1 uint8 // indexed // 索引
		Value2 uint8
	}
	abi, err := JSON(strings.NewReader(definition))
	require.NoError(t, err)
...
// TestEventIndexedWithArrayUnpack 验证当静态数组作为索引输入时解码器不会溢出。
func TestEventIndexedWithArrayUnpack(t *testing.T) {
	t.Parallel()
	definition := `[{"name": "test", "type": "event", "inputs": [{"indexed": true, "name":"value1", "type":"uint8[2]"},{"indexed": false, "name":"value2", "type":"string"}]}]`
	type testStruct struct {
		Value1 [2]uint8 // indexed // 索引
		Value2 string
	}
	abi, err := JSON(strings.NewReader(definition))
	require.NoError(t, err)
