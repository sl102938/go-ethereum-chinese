...
// 这是一个编译时断言，以确保生成的文件与其正在编译的原始包兼容。此行的编译错误可能意味着您的原始包副本需要更新。
const _ = proto.ProtoPackageIsVersion3 // please upgrade the proto package // 请升级proto包
...
// * 恢复过程的类型。这些应该用作位掩码，例如“RecoveryDeviceType_ScrambledWords | RecoveryDeviceType_Matrix` 列出了主机支持的每种方法。
...
// 请注意，为了向后兼容，每个实现都必须支持 ScrambledWords；没有办法不支持。
...
// * 请求：将设备重置为默认状态并询问设备详细信息 @start @next 功能
...
// * 请求：询问设备详细信息（无设备重置）@start @next 功能
...
// * 请求：清除会话（删除缓存的 PIN、密码等）。 @开始 @下一个 成功
...
// * 请求：更改设备的语言和/或标签 @start @next Success @next Failure
...
// * 请求：设置设备的标志 @start @next Success @next Failure
...
// * 请求：启动设置/更改/删除 PIN 的工作流程 @start @next Success @next Failure
...
// * 请求：测试设备是否存活，设备在Success响应中发回消息@start @next Success
type Ping struct {
...
// * 请求：中止需要用户交互的上一个操作 @start @next Failure
type Cancel struct {
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
...
// * 请求：请求硬件RNG生成的随机数据样本。可用于测试。 @start @next 熵 @next 失败
...
// * 请求：请求设备擦除所有敏感数据和设置 @start @next Success @next Failure
...
// * 请求：从计算机加载种子及相关内部设置 @start @next Success @next Failure
...
// * 请求：要求设备进行涉及用户交互的初始化 @start @next EntropyRequest @next Failure
...
// * 请求：如果未使用 ResetDevice @start @next Success 备份，则执行设备种子的备份
...
// * 请求：启动恢复工作流程，要求用户输入助记符的特定单词，即使在不受信任的计算机上也可以安全地恢复设备。 @开始@下一个WordRequest
...
// * 响应：设备正在等待用户输入助记词。其位置仅显示在设备的内部显示屏上。 @下一个WordAck
...
// * 请求：计算机回复助记符@next WordRequest @next Success @next Failure
type WordAck struct {
...
var fileDescriptor_0c720c20d27aa029 = []byte{
	...
	// gzip 压缩的 FileDescriptorProto 的 1393 字节
