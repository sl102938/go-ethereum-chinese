...
// * 响应：设备正在等待按下硬件按钮。 @auxstart @next ButtonAck
...
// * 响应：设备要求计算机显示 PIN 矩阵并等待使用此矩阵方案编码的 PIN @auxstart @next PinMatrixAck
...
// * 响应：设备等待加密密码 @auxstart @next PassphraseAck
...
// * 请求：发送回密码@next PassphraseStateRequest
...
// * 响应：设备等待密码状态@next PassphraseStateAck
...
// * 表示 BIP32（分层确定性）节点的结构 用于将私钥导入设备并将公钥导出设备 @embed
...
var fileDescriptor_aaf30d059fdbc38d = []byte{
	...
	// gzip 压缩的 FileDescriptorProto 的 846 字节
