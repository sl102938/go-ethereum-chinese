...
// * 请求：向设备询问address_n路径对应的公钥 @start @next EthereumPublicKey @next Failure
...
// * 请求：向设备询问address_n路径对应的以太坊地址 @start @next EthereumAddress @next Failure
...
// * 请求：要求设备签署交易 从协议的角度来看，所有字段都是可选的。如果缺少，每个字段默认值为“0”。注意：前最多 1024 字节的数据必须作为该消息的一部分进行传输。 @start @next EthereumTxRequest @next Failure
...
// * 响应：设备从交易有效负载中请求更多数据，或返回签名。如果设置了 data_length，设备将等待更多字节的有效负载。否则，signature_* 字段包含计算出的交易签名。所有三个字段都将出现。 @end @next EthereumTxAck
...
// * 请求：要求设备签署消息 @start @next EthereumMessageSignature @next Failure
...
// * 请求：要求设备验证消息 @start @next Success @next Failure
...
var fileDescriptor_cb33f46ba915f15c = []byte{
	...
	// gzip 压缩的 FileDescriptorProto 的 593 字节
